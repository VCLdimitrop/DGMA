...
from models.decoder_refiner.decoder_refiner import Decoder_Refiner
...
def train_net(cfg):
    ...
    # load model
    encoder = Encoder(cfg)
    decoder_refiner = Decoder_Refiner(cfg)
    merger = Merger(cfg)
    init_epoch, best_iou, best_epoch, encoder, decoder_refiner, merger, cfg = \
        pipeline.setup_network(cfg, encoder, decoder_refiner, merger)

    # Set up solver
    encoder_solver, decoder_refiner_solver, merger_solver = pipeline.solver(cfg, encoder, decoder_refiner, merger)
    ...
    if cfg.TRAIN.LR_scheduler == 'MilestonesLR':
        ...
        decoder_refiner_lr_scheduler = \
            torch.optim.lr_scheduler.MultiStepLR(
                decoder_refiner_solver, milestones=[lr for lr in cfg.TRAIN.MILESTONESLR.decoder_refiner_LR_MILESTONES],
                gamma=cfg.TRAIN.MILESTONESLR.GAMMA)
        merger_lr_scheduler = \
            torch.optim.lr_scheduler.MultiStepLR(
    ...
    for epoch_idx in range(init_epoch, cfg.TRAIN.NUM_EPOCHS):
        ...
        for batch_idx, (taxonomy_names, sample_names, rendering_images, ground_truth_volumes) \
            ...
            # Train the encoder, decoder_refiner, and merger
            # encoder
            ...
            # decoder_refiner
            generated_volumes = decoder_refiner(context).squeeze(dim=1)
            ...
            # Gradient decent
            encoder.zero_grad()
            decoder_refiner.zero_grad()
            merger.zero_grad()

            loss.backward()

            encoder_solver.step()
            decoder_refiner_solver.step()
            merger_solver.step()
            ...
            if torch.distributed.get_rank() == 0:
                ...
                if batch_idx == 0 or (batch_idx + 1) % cfg.TRAIN.SHOW_TRAIN_STATE == 0:
                    ...
                    print('LearningRate:\tencoder: %f | decoder_refiner: %f | merger: %f | %d_views_rendering' %
                              (encoder_lr_scheduler.optimizer.param_groups[0]['lr'],
                               decoder_refiner_lr_scheduler.optimizer.param_groups[0]['lr'],
        ...
        decoder_refiner_lr_scheduler.step()
        ...
        iou = test_net(cfg, epoch_idx + 1, val_data_loader, val_file_num, val_writer, encoder, decoder_refiner, merger)
        ...
        if torch.distributed.get_rank() == 0:
            if (epoch_idx + 1) % cfg.TRAIN.SAVE_FREQ == 0 or iou > best_iou:
                ...
                checkpoint = {
                    ...
                    'decoder_refiner_state_dict': decoder_refiner.state_dict(),
