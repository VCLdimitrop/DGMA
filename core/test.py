...
def test_net(cfg,
    ...
    # Set up networks
    if decoder_refiner is None or encoder is None or Merger is None:
        encoder = Encoder(cfg)
        decoder_refiner = Decoder_Refiner(cfg)
        merger = Merger(cfg)

        encoder, decoder_refiner, merger, epoch_idx = \
    ...
    for sample_idx, (taxonomy_id, sample_name, rendering_images, ground_truth_volume) in for_tqdm:
        ...
        with torch.no_grad():
            ...
            # Test the encoder, decoder_refiner and merger
            # encoder
            ...
            # decoder_refiner
            generated_volume = decoder_refiner(context).squeeze(dim=1)
